...
echo "Creating epic..."
EPIC_JSON=$(bd create "Phase 2: Bidirectional sync and state tracking" \
  -t epic -p 0 \
...
DB_JSON=$(bd create "Add SQLite tracking database" \
...
DIFF_JSON=$(bd create "Implement diff detection" \
  -t task -p 0 --parent "$EPIC_ID" \
...
SYNC_JSON=$(bd create "Build sync command" \
  -t task -p 1 --parent "$EPIC_ID" \
  -d "Create cmd/strung/sync.go subcommand for incremental sync" \
  --design "Read UBS JSON from stdin, compute diffs, create/update/close Beads issues via bd CLI. Flags: --db-path, --auto-close, --dry-run, --min-severity" \
  --acceptance "Works: ubs --format=json | strung sync --db-path=.strung.db" \
  --json)
SYNC_ID=$(echo "$SYNC_JSON" | jq -r '.id')
echo "✓ Sync: $SYNC_ID"

RESOLUTION_JSON=$(bd create "Add resolution detection" \
  -t task -p 1 --parent "$EPIC_ID" \
  -d "Implement auto-close for resolved findings" \
  --design "When finding disappears from scan, optionally close corresponding Beads issue via bd close. Controlled by --auto-close flag" \
...
ENRICHMENT_JSON=$(bd create "Add metadata enrichment" \
...
TESTS_JSON=$(bd create "Add integration tests for sync workflow" \
  -t task -p 2 --parent "$EPIC_ID" \
...
DOCS_JSON=$(bd create "Update documentation for Phase 2" \
  -t task -p 2 --parent "$EPIC_ID" \
...
ERRORS_JSON=$(bd create "Add error handling and transaction safety" \
  -t task -p 1 --parent "$EPIC_ID" \
...
bd dep add "$DIFF_ID" "$DB_ID" --type blocks
bd dep add "$SYNC_ID" "$DIFF_ID" --type blocks
bd dep add "$SYNC_ID" "$DB_ID" --type blocks
bd dep add "$RESOLUTION_ID" "$SYNC_ID" --type blocks
bd dep add "$ENRICHMENT_ID" "$SYNC_ID" --type blocks
bd dep add "$TESTS_ID" "$RESOLUTION_ID" --type blocks
bd dep add "$TESTS_ID" "$ENRICHMENT_ID" --type blocks
bd dep add "$DOCS_ID" "$SYNC_ID" --type blocks
bd dep add "$DOCS_ID" "$RESOLUTION_ID" --type blocks
bd dep add "$DOCS_ID" "$ENRICHMENT_ID" --type blocks
bd dep add "$ERRORS_ID" "$SYNC_ID" --type blocks
bd dep add "$ERRORS_ID" "$DB_ID" --type blocks
bd dep add "$TESTS_ID" "$ERRORS_ID" --type blocks
